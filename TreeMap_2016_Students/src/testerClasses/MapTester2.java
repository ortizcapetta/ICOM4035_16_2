...
public class MapTester2 {
	...
	public static void main(String[] args) {
		...
		/**
		 * Testing Exercise 2 with the methods ShowMapValues and showMapKeys
		 */
		showMap("Original Map", map);
		showMapEntries("Original Map Entries",map);
		showMapValues("Map values",map); 
		showMapKeys("Map keys",map);
		
		mapInsert(19, "nineteen 2", map); 
		showMap("Map content after adding 19 ...", map); 
		showMapEntries("Map entries after adding 19",map);
		showMapValues("Map values after adding 19",map);
		showMapKeys("Map keys after adding 19",map);
		
		map.remove(13); 
		showMap("Map content after removing 13 ...", map); 
		showMapValues("Map values after removing 13",map);
		showMapKeys("Map keys after removign 13",map);
