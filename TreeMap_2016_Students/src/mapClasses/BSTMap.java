...
public class BSTMap<K, V> implements Map<K, V> {
	...
	public V put(K key, V value) {
		...
		//...
		MapEntry<K,V> ent = new MapEntry<K,V>(key,value);
		Position<Entry<K,V>> res = tree.getPosition(ent);
		if(res==null){
			
			tree.addElement(ent); //add the entry if it did not exist
			return null; //previous value, in this case null
		}else{
			V val = res.getElement().getValue();
			((MapEntry<K, V>) res.getElement()).setValue(value);  //replace the value of the entry with the new value given
			
			return val; //previous value
	...
	public V remove(K key) {
		...
		// them to instance field tree.... EXERCISE 1
		//...
		Position<Entry<K,V>> res = tree.getPosition(new MapEntry<>(key, null)); //find the key in the tree
		if(res==null){
			return null; //if there was no such key in the tree
		}
		else{
			V val = res.getElement().getValue();
			tree.removeElement(res.getElement());  //delete the entry and return its value
			return val;
		}
		    // for the moment...
	}
