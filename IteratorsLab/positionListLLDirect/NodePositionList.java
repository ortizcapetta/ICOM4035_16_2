...
public class NodePositionList<T> implements PositionList<T> {
	...
	//Used for Activity 5 
	public NodePositionList(String nameIterator) {
		header = new DNode<T>(null, null, null, this); 
		trailer = new DNode<T>(null, header, null, this);
		header.setNext(trailer); 
		count = 0; 
	...
	public Iterable<Position<T>> positions() {
		NodePositionList<Position<T>> pList = new NodePositionList<Position<T>>(nameIterator); 
		try { 
			if (!this.isEmpty()) { 
	...
	/**
	 * More iterators can be added here, if not specified it will iterate normally
	 * but if there is an additional String name in the constructor of the NodePositionList object
	 * it will verify if there is an iterator available
	 * 
	 */
	public Iterator<T> iterator() {  //ACTIVITY NUMBER 5
		if(nameIterator.equalsIgnoreCase("backwards")){
